...
# Determine if this is actual production based on the branch name
RUN if [ "$BRANCH_NAME" = "main" ]; then \
  export IS_ACTUAL_PRODUCTION=true; \
  else \
  export IS_ACTUAL_PRODUCTION=false; \
  fi && \
  echo "IS_ACTUAL_PRODUCTION=$IS_ACTUAL_PRODUCTION" > /app/.env
